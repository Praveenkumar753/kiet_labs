...
const LabAvailability = () => {
    ...
    const fetchLabAvailability = async () => {
        ...
        try {
            ...
            for (const college of colleges) {
                ...
                const collegeYears = await axios.get('http://localhost:5000/api/registration/years', { params: { college: college } });

                for (const year of collegeYears.data) {
                    const filteredOriginalData = originalDataResponse.data.filter(student =>
                        validColleges.includes(student.COL) && student.YEAR.toString() === year.toString()
                    );
                    const departments = await axios.get('http://localhost:5000/api/registration/departments', { params: { college: college, year: year } });

                    for (const department of departments.data) {
                        const types = await axios.get('http://localhost:5000/api/registration/types', { params: { college: college, year: year, department: department } });
                        ...
                        for (const type of types.data) {
                            ...
                            availableLabs.forEach(lab => {
                                const labApplicants = registrationResponse.data.filter(reg =>
                                    validColleges.includes(reg.college) && reg.department === department && reg.type === type && reg.selectedLab === lab && reg.YEAR.toString() === year.toString()
                                );

                                availability.push({
                                    college, year, department, type, lab, totalStudents, totalAvailable: labCapacity, applicants: labApplicants.length, vacant: Math.max(0, labCapacity - labApplicants.length)
                                });
                            });
                        }
                    }
            ...
            availability.sort((a, b) =>
                a.college.localeCompare(b.college) || a.year - b.year ||
            ...
            setUniqueDepartments([...new Set(availability.map(item => item.department))]);
            setUniqueTypes([...new Set(availability.map(item => item.type))]);
            setUniqueYears([...new Set(availability.map(item => item.year.toString()))]);
    ...
    const filteredData = labAvailability.filter(item =>
        item.college === selectedCollege && item.year.toString() === selectedYear &&
        item.department === selectedDepartment && item.type === selectedType
    );
    ...
    return (
        <div className="stats-container">
            <div className="stats-content">
                ...
                {selectedCollege && selectedYear && selectedDepartment && selectedType && (
                    <Link to="/register"><button className="stats-apply-button">Apply</button></Link>
                )}
            </div>
