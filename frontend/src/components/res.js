...
const FormResponses = () => {
    ...
    useEffect(() => {
        const fetchResponses = async () => {
            try {
                const res = await axios.get('http://localhost:5000/api/formresponses');
    ...
    const handleSaveEdit = async (id) => {
        try {
            ...
            for (const [key, value] of relevantEdits) {
                ...
                await axios.put(`http://localhost:5000/api/formresponses/${rowId}/${field}`, { newValue: value });
            }
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this response?')) {
            try {
                await axios.delete(`http://localhost:5000/api/formresponses/${id}`);
